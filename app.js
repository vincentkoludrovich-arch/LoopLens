...
function buildAnalysis() {
  ...
  return {
    ...
    confidenceSummary:
      `${confidenceLabel(confidence)} Confidence is based on item type, material profile, residue level, image signal, and deployment context.`,
    chartCaption:
...
els.upload.addEventListener("change", (event) => {
  ...
  readImageMeta(file);
  els.scanSignal.textContent = file ? `${file.name} ready for analysis` : "No photo analyzed";
  els.scanConfidence.textContent = file ? "Ready" : "Ready";
});
